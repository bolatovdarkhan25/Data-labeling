<div class="bank-card">
    @if ($displayModal)
        ...
        <div class="modal" tabindex="-1" role="dialog" style="display: block; background-color: rgba(0,0,0,0.4);">
            <div class="modal-dialog" role="document" style="top: 50px; width: 700px">
                <div class="modal-content" style="border-radius: 10px; width: 700px; right: 100px">
                    <button type="button" class="close d-flex justify-content-end mt-1" wire:click.prevent="close">
                            ...
                            <form id="card-form">
                                <div class="form-container">
                                    <div class="field-container">
                                        ...
                                        <input id="name" data-cp="name" maxlength="20" type="text">
                                    </div>
                                    <div class="field-container">
                                        <label for="cardnumber" class="mb-0">Card Number</label>
                                        <input id="cardnumber" data-cp="cardNumber" type="text" pattern="[0-9]*" inputmode="numeric">
                                        <svg id="ccicon" class="ccicon" width="750" height="471" viewBox="0 0 750 471" version="1.1" xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink">
                                        </svg>
                                    </div>
                                    <div class="field-container">
                                        <label for="expirationmonth" class="mb-0">Expiration month</label>
                                        <input id="expirationmonth" data-cp="expDateMonth" type="text" pattern="[0-9]*" inputmode="numeric">
                                    </div>
                                    <div class="field-container">
                                        <label for="expirationyear" class="mb-0">Expiration year</label>
                                        <input id="expirationyear" data-cp="expDateYear" type="text" pattern="[0-9]*" inputmode="numeric">
                                    </div>
                                    <div class="field-container">
                                        <label for="securitycode" class="mb-0">Security Code</label>
                                        <input id="securitycode" data-cp="cvv" type="password" pattern="[0-9]*" inputmode="numeric">
                                    </div>
                                </div>
                            </form>
                        </div>
                        <div class="text-center">
                            <button id="pay-button" class="btn btn-outline-success w-25" @if($disable) disabled @endif>
                                Pay
                            </button>
                        </div>
                    </div>
            ...
            expirationmonth_mask.on('accept', function () {
                if (expirationmonth_mask.value.length == 0) {
                    if (expirationyear_mask.value.length == 0) {
                        ...
                        document.getElementById('svgexpire').innerHTML = '/' + expirationyear_mask.value;
                    }
                } else {
                    document.getElementById('svgexpire').innerHTML = expirationmonth_mask.value + '/' + expirationyear_mask.value;
                }
            });
            ...
            expirationyear_mask.on('accept', function () {
                if (expirationyear_mask.value.length == 0) {
                    if (expirationmonth_mask.value.length == 0) {
                        ...
                        document.getElementById('svgexpire').innerHTML = expirationmonth_mask.value;
                    }
                } else {
                    document.getElementById('svgexpire').innerHTML = expirationmonth_mask.value + '/' + expirationyear_mask.value;
                }
...
//On Focus Events
            ...
            let checkout = new cp.Checkout(
                'pk_d33f9e2a27bf18ec2dc4e99b06b5b',
                document.getElementById('card-form')
            );

            payButton.addEventListener('click', function () {
                let cardHolderName = name.value;

                var result = checkout.createCryptogramPacket();

                if (result.success) {
                    let lw = new Livewire();
                    lw.emit('cryptogramCreated', {
                        cryptogram: result.packet,
                        name: cardHolderName
                    });

                    lw.on('subscribed', function () {
                        if (window.confirm('Successfully subscribed!'))
                        {
